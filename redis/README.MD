...
编辑`/usr/local/redis-6.0.8/redis.conf`配置文件；看情况修改

```
daemonize  yes  ## 后台运行
logfile "/var/logs/redis/redis.log"  ## 日志文件
...
[Service]
Type=forking
ExecStart=/usr/local/redis-6.0.8/src/redis-server /usr/local/redis-6.0.8/redis.conf
PrivateTmp=true
[Install]
...
activerehashing yes
# 指定含其它的配置文件，可以在同一主机上多个Redis实例之间使用同一份配置文件，而同时各个实例又拥有自己的特定配置文件
